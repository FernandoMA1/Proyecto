...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $confirmpassword) {
        header('Location: apps-crm-contact.php?msg=password_coincidencia');
        exit;
    } else {
        ...
        if ($stmt_email->num_rows > 0) {
            header('Location: apps-crm-contact.php?msg=email_en_uso');
            exit;
        } elseif ($stmt_tel->num_rows > 0) {
            header('Location: apps-crm-contact.php?msg=telefono_en_uso');
            exit;
        } else {
            ...
            if ($stmt->execute()) {
                header('Location: apps-crm-contact.php?msg=registro_exitoso');
            } else {
                header('Location: apps-crm-contact.php?msg=registro_fallido');
                exit;
            }
            $stmt->close();
        }
